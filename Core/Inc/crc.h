...
//---------------------------------------------------------------------------
/// Таблица для расчета CRC8-CCITT для полинома x^8 + x^2 + x + 1
...
/// Таблица для расчета CRC16-CCITT для полинома x^16 + x^12 + x^5 + 1
...
/// Таблица для расчета CRC32-IEEE для полинома x^32 + x^26 + x^23 + x^22 + x^16 + x^12 + x^11
...
/// \return Значение CRC-8-CCITT входных данных
uint8_t Crc8(const uint8_t *Block, uint8_t len);

/// \brief Инициализация контекста для расчета CRC-8-CCITT
/// \return Контекст CRC-8
uint8_t Crc8Init(void);


/// \brief Pасчет CRC-8-CCITT для дополнительного блока данных
/// \param Context - Указатель на контекст CRC-8
/// \param Block - Указатель на буфер данных
/// \param len - Длина буфера
void Crc8Update(uint8_t *Context, const uint8_t *Block, uint8_t len);


/// \brief Pасчет CRC-8-CCITT для байта данных
/// \param Context - Указатель на контекст CRC-8
/// \param Value - Байт данных
void Crc8UpdateByte(uint8_t *Context, uint8_t Value);


/// \brief Завершение расчета CRC-8-CCITT
/// \param Context - Контекст CRC-8
/// \return Значение CRC-8-CCITT
uint8_t Crc8Final(uint8_t Context);


/// \brief Расчет CRC-16-CCITT
///
/// Name  : CRC-16-CCITT
...
/// \param Block - Указатель на буфер данных
/// \param len - Длина буфера
/// \return Значение CRC-16-CCITT входных данных
uint16_t Crc16(const uint8_t *Block, uint16_t len);


/// \brief Инициализация контекста для расчета CRC-16-CCITT
/// \return Контекст CRC-16-CCITT
uint16_t Crc16Init(void);


/// \brief Pасчет CRC-16-CCITT для дополнительного блока данных
/// \param Context - Указатель на контекст CRC-16
/// \param Block - Указатель на буфер данных
/// \param len - Длина буфера
void Crc16Update(uint16_t *Context, const uint8_t *Block, uint16_t len);


/// \brief Pасчет CRC-16-CCITT для байта данных
/// \param Context - Указатель на контекст CRC-16
/// \param Value - Байт данных
void Crc16UpdateByte(uint16_t *Context, uint8_t Value);


/// \brief Завершение расчета CRC-16-CCITT
/// \param Context - Указатель на контекст CRC-16
/// \return Значение CRC-16-CCITT
